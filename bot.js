...
for(i in arrayAccountsTxt) {
    ...
    accounts[i] = [];
    accounts[i].push({username: username, password: password});
}
			...
			if (res.eresult !== Steam.EResult.OK) {
				...
				if (GC) { 
					IntervalInt = setInterval(function() {
						GC.send({ msg: ClientHello, proto: {} }, new Protos.CMsgClientHello({}).toBuffer());
					}, 2000);
...
function sendReport(GC, Client, account_name) {
    ...
    GC.send({ msg: Protos.ECsgoGCMsg.k_EMsgGCCStrike15_v2_ClientReportPlayer, proto: {} }, new Protos.CMsgGCCStrike15_v2_ClientReportPlayer({ accountId: account_id, matchId: 8, rptAimbot: 2, rptWallhack: 3, rptSpeedhack: 4, rptTeamharm: 5, rptTextabuse: 6, rptVoiceabuse: 7 }).toBuffer());
}
